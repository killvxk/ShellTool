...
#include "SATAReadWriteCompare.h"
#include "Ps2Keyboard.h"

EFI_HANDLE gShellSataHiiHandle = NULL;

extern UINT8      SataReadwritecompareStrings[];
  ...
  do
  {
  	...
  	for(i=0;i<Opera_Count;i++)
    {
	     ...
	     if(my_memcpy(Write_Buffer,Read_Buffer,BlockIo->Media->BlockSize)!=0)
	     {
            ...
            if(j==BlockIo->Media->BlockSize)
            {
            	DEBUG((EFI_D_ERROR,"\rCompare at 0x%llx Success!\n",(Start_Lba+i)));
            }
	     }
...
DEBUG((EFI_D_ERROR,"Write/Read/Compare OK!\n"));
  ...
  )
{
  ...
  gShellSataHiiHandle = HiiAddPackages (&gShellSataHiiGuid, gImageHandle, SataReadwritecompareStrings, NULL);
