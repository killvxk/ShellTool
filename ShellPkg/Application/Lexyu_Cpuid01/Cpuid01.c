...
EFI_STATUS 
CollectCpuInfo (
  IN  FRAMEWORK_EFI_MP_SERVICES_PROTOCOL  *FrameworkMpService
  )
{
  EFI_STATUS                Status;
  ...
  Status = FrameworkMpService->GetGeneralMPInfo (
                                  FrameworkMpService,
                                  &NumberOfProcessors,
                                  NULL,
                                  NULL,
  ...
  for (ProcessorIndex = 0; ProcessorIndex < NumberOfProcessors; ProcessorIndex++) {
    Status = FrameworkMpService->GetProcessorContext (
                                    FrameworkMpService,
                                    ProcessorIndex,
                                    &BufferSize,
    ...
    //
    if (NumberOfThreadsPerCore < (UINT32) ProcessorContextBuffer.NumberOfThreads + 1) {
      NumberOfThreadsPerCore = (UINT32) ProcessorContextBuffer.NumberOfThreads + 1;
    }

    gCpuInfoEntry[ProcessorIndex].ApicId  = (UINT64) ProcessorContextBuffer.ApicID;

    //
    // Get Status Flag of specified processor
    //
    gCpuInfoEntry[ProcessorIndex].StatusFlag = 0;
    
    if (ProcessorContextBuffer.Enabled) {
      gCpuInfoEntry[ProcessorIndex].StatusFlag |= PROCESSOR_ENABLED_BIT;
    }
    
    if (ProcessorContextBuffer.Designation == EfiCpuBSP) {
      gCpuInfoEntry[ProcessorIndex].StatusFlag |= PROCESSOR_AS_BSP_BIT;
    }
    
    if (ProcessorContextBuffer.Health.Flags.Uint32 == 0) {
      gCpuInfoEntry[ProcessorIndex].StatusFlag |= PROCESSOR_HEALTH_STATUS_BIT;
    }
    
    if (gCpuInfoEntry[ProcessorIndex].StatusFlag & PROCESSOR_AS_BSP_BIT) {
      ApGetCpuInfo ((VOID *)ProcessorIndex);
    } else {
      Status = FrameworkMpService->StartupThisAP (
                            FrameworkMpService, 
                            ApGetCpuInfo, 
                            ProcessorIndex, 
                            NULL, 
  ...
  )
{
  ...
  Status = gBS->LocateProtocol (&gFrameworkEfiMpServiceProtocolGuid, NULL, (VOID **) &FrameworkMpService);
  if(EFI_ERROR (Status)){
        Print(L"[ERROR]: Locate the Framework MP Services Protocol Fail!\n");
        return EFI_UNSUPPORTED;
  }

  Status = CollectCpuInfo(FrameworkMpService);
  ASSERT_EFI_ERROR(Status);  

  if(argc==1){
        Status = PrintCpuInfo(FrameworkMpService);
		ASSERT_EFI_ERROR(Status);  
  }else if(argc==2){
        gCpuid = (UINT32)StrHexToUintn(argv[1]);
        Status = CollectCpuId(FrameworkMpService);
        ASSERT_EFI_ERROR(Status);

        Status = PrintCpuId(FrameworkMpService);
		ASSERT_EFI_ERROR(Status);    
  }else
  {
