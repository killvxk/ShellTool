  ...
  )
{
  UINTN                         Index;
  UINTN                         StrIndex;
  BOOLEAN                       FindBootManagerMenu  = FALSE;
  UINTN                         BootManagerMenuIndex = 0;
  ...
  if(FindBootManagerMenu) {
    ASSERT (BootOption[BootManagerMenuIndex].Description != NULL);
    BootMenuData->PtrTokens[StrIndex++] = HiiSetString (
                                            gStringPackHandle, 
                                            0,
                                            BootOption[BootManagerMenuIndex].Description,
                                            NULL
                                            );
  }
  ...
  )
{
  ...
  BdsBootMgr->RefreshOption();
  BdsBootMgr->GetOption(&BootOption, &BootOptionCount);  

  BootOption = ReallocatePool(
                 BootOptionCount * sizeof(EFI_BOOT_MANAGER_LOAD_OPTION),
                 (BootOptionCount+1) * sizeof(EFI_BOOT_MANAGER_LOAD_OPTION),
                 BootOption
                 );
  ...
  while (!ExitApplication) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      case CHAR_CARRIAGE_RETURN:
        //
        // UEFI OS will not clean screen, clean boot menu before boot.
        //
        gST->ConOut->ClearScreen (gST->ConOut);
  ...
  BdsBootMgr->FreeOption(BootOption, BootOptionCount);
