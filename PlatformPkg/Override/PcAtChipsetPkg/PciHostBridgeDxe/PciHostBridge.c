...
PCI_ROOT_BRIDGE_RESOURCE_APPETURE  mResAppeture[1][1] = {
  {0, 0xff, 0x00000000, 0xFFFFFFFFFFFFFFFFULL, 0, 0xffff}		// do not check MMIO low limit.
};
            ...
            switch (Index) {
              ...
              case TypeIo:  
                ...
                Status = gDS->AllocateIoSpace (
                                EfiGcdAllocateAnySearchTopDown,     //- EfiGcdAllocateAnySearchBottomUp
                                EfiGcdIoTypeIo, 
                                BitsOfAlignment,
                                AddrLen,
                                &BaseAddress,
              ...
              case TypeMem32:
                ...
                DEBUG((EFI_D_ERROR, "PciM32 Abits:%d L:0x%X\n", BitsOfAlignment, AddrLen));
                UpdateTolumVariable(BitsOfAlignment, AddrLen); // 2016-06-17 for SOC, not update Tolum?!
                ...
                DEBUG((EFI_D_ERROR, "malloc MMIO32(%r):%lX\n", Status, BaseAddress));                 
                if (!EFI_ERROR (Status)) {
              ...
              case TypeMem64: 
                ...
                DEBUG((EFI_D_ERROR, "PciM64 Abits:%d L:0x%lX\n", BitsOfAlignment, AddrLen));
                if(AddrLen == 0){
                  break;
                } 
                
                BaseAddress = 0xFFFFFFFFFFFFFFFFULL;
                Status = gDS->AllocateMemorySpace (
                                EfiGcdAllocateMaxAddressSearchTopDown, 
                                EfiGcdMemoryTypeMemoryMappedIo, 
                                BitsOfAlignment,
                ...
                DEBUG((EFI_D_ERROR, "malloc MMIO64(%r):%lX\n", Status, BaseAddress));                 
                if (!EFI_ERROR (Status)) {
                  // We were able to allocate the PCI memory
                  RootBridgeInstance->ResAllocNode[Index].Base   = (UINTN)BaseAddress;
                  RootBridgeInstance->ResAllocNode[Index].Status = ResAllocated;
...
//        }
        ...
        case ACPI_ADDRESS_SPACE_TYPE_MEM:
          ...
          if (Ptr->AddrSpaceGranularity == 32 && Ptr->AddrLen > 0xffffffff) {
            DEBUG((EFI_D_INFO, "BadAddrLen:%lX\n", Ptr->AddrLen));            
            return EFI_INVALID_PARAMETER;
          }
  ...
  )
{
...

  {
    ...
    Status = AsiaSb->PreprocessPciController(AsiaSb, &RootBridgeInstance->Io, PciAddress, Phase);
    ASSERT_EFI_ERROR(Status);
    Status = AsiaNb->PreprocessPciController(AsiaNb, &RootBridgeInstance->Io, PciAddress, Phase);
    ASSERT_EFI_ERROR(Status);
  }
