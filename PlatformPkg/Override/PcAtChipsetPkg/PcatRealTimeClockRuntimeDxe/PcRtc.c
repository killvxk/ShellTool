  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG((EFI_D_ERROR, "Bad RTC, Load Default Date & Time.\n"));		
  ...
  } else {			// RTC OK, so check legacy century (0x32)
    if(RtcRead(RTC_ADDRESS_LEGCTY) != Century){
      DEBUG((EFI_D_ERROR, "Bad Cmos0x32, Sync it with Century\n"));				
      RtcWrite (RTC_ADDRESS_LEGCTY, Century);
    }			
  }
