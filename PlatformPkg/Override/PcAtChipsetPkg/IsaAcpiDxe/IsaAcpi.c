...
//
EFI_ISA_ACPI_RESOURCE_LIST gPcatIsaAcpiDeviceList[9] = {0};
  ...
  )
{
  ...
  if(SetupHob->Uart0Enable){
    List = &mPcatIsaAcpiCom1Device;
    List->ResourceItem[0].StartRange = gComBaseList[SetupHob->Uart0IoBaseSelection];
    List->ResourceItem[0].EndRange   = List->ResourceItem[0].StartRange + 7;
    List->ResourceItem[1].StartRange = SetupHob->Uart0IRQSelection;
    CopyMem (&gPcatIsaAcpiDeviceList[Index++], List, sizeof(*List));
  }  
  if(SetupHob->Uart1Enable){
    List = &mPcatIsaAcpiCom2Device;
    List->ResourceItem[0].StartRange = gComBaseList[SetupHob->Uart1IoBaseSelection];
    List->ResourceItem[0].EndRange   = List->ResourceItem[0].StartRange + 7;
    List->ResourceItem[1].StartRange = SetupHob->Uart1IRQSelection;
    CopyMem (&gPcatIsaAcpiDeviceList[Index++], List, sizeof(*List));
  }  
  if(SetupHob->Uart2Enable){
    List = &mPcatIsaAcpiCom3Device;
    List->ResourceItem[0].StartRange = gComBaseList[SetupHob->Uart2IoBaseSelection];
    List->ResourceItem[0].EndRange   = List->ResourceItem[0].StartRange + 7;
    List->ResourceItem[1].StartRange = SetupHob->Uart2IRQSelection;
    CopyMem (&gPcatIsaAcpiDeviceList[Index++], List, sizeof(*List));
  }      
  if(SetupHob->Uart3Enable){
    List = &mPcatIsaAcpiCom4Device;
    List->ResourceItem[0].StartRange = gComBaseList[SetupHob->Uart3IoBaseSelection];
    List->ResourceItem[0].EndRange   = List->ResourceItem[0].StartRange + 7;
    List->ResourceItem[1].StartRange = SetupHob->Uart3IRQSelection;
    CopyMem (&gPcatIsaAcpiDeviceList[Index++], List, sizeof(*List));
  }   
