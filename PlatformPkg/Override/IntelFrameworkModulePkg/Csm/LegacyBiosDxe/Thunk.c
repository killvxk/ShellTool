...
VOID IntVectorCheck(UINT8 In, CONST EFI_IA32_REGISTER_SET *Regs)
{
  UINT32      *Data32;
  UINT32      Addr;	
  UINTN       Index;
  ...
  Data32 = (UINT32*)(UINTN)0;

// INT 1fH: CGA Graphics Character Font Pointer
// INT 43H: EGA/VGA Graphics Character Font Pointer
	
  for(Index=0;Index<0x78;Index++){
    if(Index >= 0x60 && Index <= 0x66){continue;}
	  Addr = ((Data32[Index] & 0xFFFF0000)	>> 12) + (Data32[Index] & 0xFFFF);
    if(Addr < 0xF0000){
      if(Index == 0x10 || Index == 0x1F || Index == 0x43){
        if(Addr > 0xC0000 && Addr < 0xD0000){
          continue;
        }					
      }
			if(Index >= 0x68 && Index < 0x78){
        if(Addr > 0xC0000){
          continue;
        }					
      }				
      DEBUG((EFI_D_ERROR, "[Error] IntVec[0x%X]:%X\n", Index, Addr));
      HasBad = TRUE;			
    }			
  }

  if(HasBad){
    DEBUG((EFI_D_ERROR, "(%a) A:%04X B:%04X C:%04X D:%04X\n", \
			       In?"IN":"OUT", Regs->X.AX, Regs->X.BX, Regs->X.CX, Regs->X.DX));
  }		
  ...
  )
{
  ...
  DEBUG((EFI_D_INFO, "RealModeBuffer:%X\n", (UINT32)(UINTN)mThunkContext.RealModeBuffer));
