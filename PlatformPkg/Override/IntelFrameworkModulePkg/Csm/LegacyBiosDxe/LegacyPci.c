...
Bug 3076:   Update CSM32 code to force pci3.0 oprom use the pci2.0 path. 
TIME:       2011-11-14
$AUTHOR:    Wang Huiyang
$REVIEWERS:
$SCOPE:     All Platforms
$TECHNICAL:
            Update CSM32 code to force pci3.0 oprom use the pci2.0 path.  
$END------------------------------------------------------------------

Bug 2586:   SATA port4 & port5 cannot install OS with ODD.
TIME:       2011-7-27
$AUTHOR:    Peng Xianbing
$REVIEWERS:
$SCOPE:     Sugar Bay Customer Refernce Board
$TECHNICAL:
            Enable command register before InstallLegacyIrqHandler. 
$END------------------------------------------------------------------
...
LEGACY_VIDEO_ACTION                 mLocalOpromVideoAction;

typedef struct {
  UINT16 PciConfigVenID;
  UINT16 PciConfigDevID;
  UINT16 PciOpRomVenID;
  UINT16 PciOpRomDevID;
}  SpecialVgaID;

SpecialVgaID mSpecialVgaList[] = {
  {0x1002, 0x5c61, 0x1002, 0x5c63},
  {0x1002, 0x524b, 0x1002, 0x524c},
  {0x1D17, 0x3A02, 0x1106, 0x3A02},
};
...
BOOLEAN
DealWithSpecialVGAcard (
  IN     UINT16 PciConfigVendorID,
  IN     UINT16 PciConfigDeviceID,
  IN     UINT16 PciOpRomVendorId,
  IN     UINT16 PciOpRomDeviceId
)
{
  UINT8       Index;
  BOOLEAN     Match;
  ...
  Match = FALSE;
  for (Index = 0; Index < sizeof(mSpecialVgaList)/sizeof(SpecialVgaID); Index ++) {
    if (mSpecialVgaList[Index].PciConfigVenID == PciConfigVendorID &&
        mSpecialVgaList[Index].PciConfigDevID == PciConfigDeviceID &&
        mSpecialVgaList[Index].PciOpRomVenID == PciOpRomVendorId &&
        mSpecialVgaList[Index].PciOpRomDevID == PciOpRomDeviceId) {
      Match = TRUE;
      break;
    }
  }
  ...
  )
{
  ...
  while (RomHeader.Generic->Signature == PCI_EXPANSION_ROM_HEADER_SIGNATURE) {
    ...
    if (Pcir->CodeType == PCI_CODE_TYPE_PCAT_IMAGE) {
      ...
      //
      // Patch the issue that some PciExpress cards PCIR & config space device IDs do not match.
      // 
      if (!Match) {
        Match = DealWithSpecialVGAcard(VendorId, DeviceId, Pcir->VendorId, Pcir->DeviceId);
      }
      if (Match) {
  ...
  )
{
  ...
  Private->IntThunk->PciHandler.PrimaryBusMaster    = 0;
  Private->IntThunk->PciHandler.SecondaryBusMaster  = 0;
  ...
  )
{
  ...
  DisplayOpromMessage = PcdGet8 (PcdDisplayOptionRomMessage);
  ...
  DEBUG ((EFI_D_ERROR, " Shadowing OpROM init/runtime/isize = %x/%x/%x\n", InitAddress, RuntimeAddress, ImageSize));
  ...
  if (PciHandle != mVgaHandle) {
    ...
    if (DisplayOpromMessage == 0) {
      if (mLocalOpromVideoAction.OldVector == 0) {
        mLocalOpromVideoAction.OldVector = *VectorPtr;
      }
      mLocalOpromVideoAction.NewVector = 0xF000E020;//get it from csm16
      *VectorPtr = mLocalOpromVideoAction.NewVector;
    } else {
      //
  ...
  *BdaPtr   = LocalTime;
  ...
  Private->IntThunk->DispatchOpromTable.RuntimeSegment              = 0xffff;
  ...
  //
    if (PciHandle != mVgaHandle) {
      ...
      //
      if (DisplayOpromMessage == 0) {
        *VectorPtr = mLocalOpromVideoAction.OldVector;
      } else {
        //
        ...
        Regs.H.AH = 0x00;
        Regs.H.AL = VideoMode;
        LegacyBiosInt86 (&Private->LegacyBios, 0x10, &Regs);
      }
    }
  //
...
Done:
  if (DisplayOpromMessage == 0 && mLocalOpromVideoAction.OldVector) {
    *VectorPtr = mLocalOpromVideoAction.OldVector;
  }
  //
