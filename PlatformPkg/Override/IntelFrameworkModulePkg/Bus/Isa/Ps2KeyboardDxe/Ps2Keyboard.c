  ...
  )
{
  ...
  if ((KeyReadStatusRegister (ConsoleIn) & (KBC_PARE | KBC_TIM)) == (KBC_PARE | KBC_TIM)) {
    ...
    //goto ErrorExit;
    DEBUG((EFI_D_ERROR, "KbdControllerDriverStart: KBC Status register error!\n"));
  }
  ...
  // when update flash with flash tool, S3->DXE, SMI services is invaild caused by SmmRam locked
  // so we check below protocol first.
