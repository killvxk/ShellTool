...

{
  ...
  UINT8                   PortStatus;
  UINT8                   KeyboardStatusMask;
  UINT8                   MouseStatusMask;
  ...
  //
  MouseStatusMask    = (KEYBOARD_STATUS_REGISTER_TRANSMIT_TIMEOUT | KEYBOARD_STATUS_REGISTER_HAS_OUTPUT_DATA);
  KeyboardStatusMask = KEYBOARD_STATUS_REGISTER_HAS_OUTPUT_DATA;
  while (1) {
    PortStatus = KeyReadStatusRegister(ConsoleIn) & MouseStatusMask;
    if(PortStatus == KeyboardStatusMask){
      Data = KeyReadDataRegister(ConsoleIn);
      PushScancodeBufTail(&ConsoleIn->ScancodeQueue, Data);
    } else if (FeaturePcdGet(PcdPs2KeyboardRxAndDiscardPs2MouseData) && (PortStatus == MouseStatusMask)){
  ...
  )
{
  ...
  if ((ConsoleIn->LeftCtrl || ConsoleIn->RightCtrl) &&
     ...
     if (!PcdGetBool(PcdDisableKBCtrlAltDelReset)) {
  ...
  )
{
  ...
  if (Ps2Policy != NULL) {
    PERF_START (NULL,"InitHW", "PS2KB", 0);	    
    Ps2Policy->Ps2InitHardware (ConsoleIn->Handle);
    PERF_END   (NULL,"InitHW", "PS2KB", 0);	    
  }
  //
  ...
  if(ExtendedVerification){
    PERF_START (NULL,"CC", "PS2KB", 0);		
    Rc = CheckKeyboardConnect(ConsoleIn);
    PERF_END   (NULL,"CC", "PS2KB", 0);		
  } else {
    Rc = FALSE;
  }	
