...
UINT32 GetMtcFromCmos(VOID)
{
  ...
  Data32.Uint8.Data[0] = CmosRead(MTC_CMOS_OFFSET);
  Data32.Uint8.Data[1] = CmosRead(MTC_CMOS_OFFSET+1);
  Data32.Uint8.Data[2] = CmosRead(MTC_CMOS_OFFSET+2);
  Data32.Uint8.Data[3] = CmosRead(MTC_CMOS_OFFSET+3); 

  gBS->RestoreTPL (OldTpl);  
  
  return Data32.Uint32;
}

VOID SetMtcToCmos(UINT32 Mtc)
{
  DATA_32_4  NewMtc;
  DATA_32_4  OldMtc;
  EFI_TPL    OldTpl;
  ...
  NewMtc.Uint32 = Mtc;
  OldMtc.Uint32 = GetMtcFromCmos();

  OldTpl = gBS->RaiseTPL(TPL_HIGH_LEVEL);   
  
  if(NewMtc.Uint8.Data[0] != OldMtc.Uint8.Data[0]){
    CmosWrite(MTC_CMOS_OFFSET, NewMtc.Uint8.Data[0]);
  }  
  if(NewMtc.Uint8.Data[1] != OldMtc.Uint8.Data[1]){
    CmosWrite(MTC_CMOS_OFFSET+1, NewMtc.Uint8.Data[1]);
  }  
  if(NewMtc.Uint8.Data[2] != OldMtc.Uint8.Data[2]){
    CmosWrite(MTC_CMOS_OFFSET+2, NewMtc.Uint8.Data[2]);
  }  
  if(NewMtc.Uint8.Data[3] != OldMtc.Uint8.Data[3]){
    CmosWrite(MTC_CMOS_OFFSET+3, NewMtc.Uint8.Data[3]);
  }  
  ...
  )
{
  ...
  HighCount = GetMtcFromCmos();
  if(HighCount == 0xFFFFFFFF){
    HighCount = 0;
  }  
  mEfiMtc = LShiftU64(HighCount, 32);
  MonotonicCounterDriverGetNextHighMonotonicCount (&HighCount);
