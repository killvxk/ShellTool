  ...
  if (Status == EFI_SUCCESS) {
    ...
    while (1) {
      if (Index == 0) {
        ...
        if (EFI_ERROR (Status)) {
          DEBUG ((EFI_D_INFO, "Capsule -- capsule variable not set\n"));
  ...
  )
{
  ...
  GetS3Cr3Stack(&S3Cr3, &S3StackBase, &S3StackSize);

  LongModeBuffer->PageTableAddress = S3Cr3;
  LongModeBuffer->StackBaseAddress = S3StackBase;
  LongModeBuffer->StackSize        = S3StackSize;
