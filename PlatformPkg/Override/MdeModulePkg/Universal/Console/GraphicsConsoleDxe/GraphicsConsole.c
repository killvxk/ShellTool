  ...
  )
{
  ...
  UINT8                                *EdidData;
  UINT32                               NativeModeHorizontal;
  UINT32                               NativeModeVertical;
  UINTN                                NativeMode;
  UINTN                                SameRatioMode;  
  UINTN                                MinimalMode;
  UINT32                               SameRatioHighHorizontal;
  UINT32                               SameRatioHighVertical;
  UINTN                                Columns;
  UINTN                                Rows;
  UINTN                                NewMode;
  BOOLEAN                              SetMaxMode;	
  ...
  ModeNumber = 0;
  EdidFound  = FALSE;
  NativeModeHorizontal = 0;
  NativeModeVertical   = 0;
  NativeMode = (UINTN) -1;
  SameRatioMode = (UINTN) -1;
  MinimalMode = (UINTN) -1;
  SameRatioHighHorizontal = 0;
  SameRatioHighVertical   = 0;
  ...
  if (Private->GraphicsOutput != NULL) {
    ...
    if ((HorizontalResolution == 0x0) || (VerticalResolution == 0x0)) {
      //
      // Find the native resolution in EDID.
      //    
      Status = gBS->OpenProtocol (
                      Controller,
      ...
      if ((!EFI_ERROR (Status)) && (EdidActive->SizeOfEdid != 0) && (EdidActive->Edid != NULL)) {
        //
        // Valid EDID is provided, find native resolution in EDID.DetailedTiming[0]
        //
        EdidFound = TRUE;
        Edid = (VESA_BIOS_EXTENSIONS_EDID_DATA_BLOCK *) EdidActive->Edid;
        //
        // Parse the Detailed Timing data
        //
        EdidData = &Edid->DetailedTimingDescriptions[0];
        if (!((EdidData[0] == 0x0) && (EdidData[1] == 0x0))) {
          //
          ...
          //
          NativeModeHorizontal = ((UINT16)(EdidData[4] & 0xF0) << 4) | (EdidData[2]);
          NativeModeVertical   = ((UINT16)(EdidData[7] & 0xF0) << 4) | (EdidData[5]);
            DEBUG ((EFI_D_INFO, "GraphicsConsole Native resolution in Detailed Timing [0]: %d x %d\n",
                    NativeModeHorizontal, NativeModeVertical));
        }
      }
      ...
      for (ModeIndex = 0; ModeIndex < MaxMode; ModeIndex++) {
        ...
        if (!EFI_ERROR (Status)) {
          //
          // Find native resolution mode number and resolution
          //
          if ((NativeModeHorizontal != 0) && (NativeModeVertical != 0)) {
            if ((Info->HorizontalResolution == NativeModeHorizontal) && 
                (Info->VerticalResolution == NativeModeVertical)) {
              NativeMode           = ModeIndex;
              //
              // Find the native resolution in EDID and native resolution is supported by Graphics Ouput Protocol,
              // get out of the loop and use native resolution.
              //
              break;
            } else if ((Info->HorizontalResolution * NativeModeVertical) ==
                       (Info->VerticalResolution * NativeModeHorizontal)) {
              //
              // Find the same ratio with native resolution
              //
              if ((Info->HorizontalResolution > SameRatioHighHorizontal) &&
                  (Info->VerticalResolution > SameRatioHighVertical)) {
                //
                // Find the higher one among all the resolution with the same ratio
                //
                SameRatioHighHorizontal = Info->HorizontalResolution;
                SameRatioHighVertical = Info->VerticalResolution;
                SameRatioMode = ModeIndex;
              }
            }
          }
          //
          // Find 1024x768 resolution as minimal resolution
          //
          if ((Info->HorizontalResolution == 1024) && (Info->VerticalResolution == 768)) {
            MinimalMode = ModeIndex;
          }
          FreePool (Info);
        }
      }
      if (NativeMode != (UINTN) -1) {
        //
        // Set native resolution if it exists
        //        
        ModeNumber = (UINT32) NativeMode;
        HorizontalResolution = NativeModeHorizontal;
        VerticalResolution   = NativeModeVertical;
        DEBUG ((EFI_D_INFO, "GraphicsConsole native mode is set!\n"));
      } else if (SameRatioMode != (UINTN) -1) {
        //
        // Fall back to same ratio with native resolution
        //
        ModeNumber = (UINT32) SameRatioMode;
        HorizontalResolution = SameRatioHighHorizontal;
        VerticalResolution   = SameRatioHighVertical;
        DEBUG ((EFI_D_INFO, "Elvin: GraphicsConsole same ratio mode is set!\n"));
      } else if (MinimalMode != (UINTN) -1) {
        //
        // Fall back to 1024x768 resolution
        //
        ModeNumber = (UINT32) MinimalMode;  
        HorizontalResolution = 1024;
        VerticalResolution   = 768;
        DEBUG ((EFI_D_INFO, "GraphicsConsole 1024x768 mode is set!\n"));
      } else {
        Status = EFI_UNSUPPORTED;
        DEBUG ((EFI_D_INFO, "GraphicsConsole ERROR! no mode is set!\n"));
        goto Error;
      }
    } else {
      //
      ...
      if (EFI_ERROR (Status)) {
        ...
        DEBUG((EFI_D_INFO, "(800x600) %r MaxMode:%d %dx%d\n", \
                           Status, Mode->MaxMode, \
...
//-       }
  ...
  DEBUG ((EFI_D_INFO, "Elvin - GraphicsConsole final resolution: %d x %d\n",
  ...
  if(PcdGet32(PcdConOutColumn)==0 || PcdGet32(PcdConOutRow)==0){
    SetMaxMode = TRUE;
    NewMode    = MaxMode-1;
  } else {
    SetMaxMode = FALSE;  
  ...
  //
  Status = GraphicsConsoleConOutSetMode (&Private->SimpleTextOutput, NewMode);
  if (EFI_ERROR (Status)) {
    goto Error;
  }

  if(SetMaxMode){
    Columns = Private->ModeData[MaxMode-1].Columns;
    Rows    = Private->ModeData[MaxMode-1].Rows;
    PcdSet32 (PcdConOutColumn, (UINT32)Columns);
    PcdSet32 (PcdConOutRow, (UINT32)Rows);
    DEBUG ((EFI_D_INFO, "GraphicsConsole:%d * %d\n", 	Columns, Rows));
  }
  ...
  )
{
  ...
  for (ModeNumber = 0; ModeNumber < MaxMode; ModeNumber++) {
    ...
    if (!EFI_ERROR (Status)) {
      DEBUG((EFI_D_INFO, "Support:%d x %d\n", Info->HorizontalResolution, Info->VerticalResolution));
