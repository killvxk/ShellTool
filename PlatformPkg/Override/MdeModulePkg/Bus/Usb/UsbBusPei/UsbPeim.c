...
static EFI_PEI_NOTIFY_DESCRIPTOR mPeiUsbEnumerationNotifyList[] = {
  {
    EFI_PEI_PPI_DESCRIPTOR_NOTIFY_CALLBACK,
    &gPeiUsbHostControllerPpiGuid,
    PeiUsbEnumerationUsbHostController
  },
  {
    (EFI_PEI_PPI_DESCRIPTOR_NOTIFY_CALLBACK | EFI_PEI_PPI_DESCRIPTOR_TERMINATE_LIST),
    &gPeiUsb2HostControllerPpiGuid,
    PeiUsbEnumerationUsb2HostController
  }
};
  ...
  )
{
  ...
  //
  // Register a call-back function for Usb Enumeration.
  //
  Status = (**PeiServices).NotifyPpi (PeiServices, &mPeiUsbEnumerationNotifyList[0]);
  DEBUG ((EFI_D_INFO, "PeimInitializeUsb NotifyPpi Status = %r\n", Status));
  return Status;
}
  ...
  )
{
  ...
  UsbHcPpi = (PEI_USB_HOST_CONTROLLER_PPI *)Interface;
  ...
  )
{
  ...
  Usb2HcPpi = (PEI_USB2_HOST_CONTROLLER_PPI *)Interface;
