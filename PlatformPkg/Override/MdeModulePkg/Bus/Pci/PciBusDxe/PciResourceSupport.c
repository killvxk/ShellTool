...
BOOLEAN  IsDualVGAPrimaryIgd()
{
  UINT32 SystemMiscCfg;
  UINT32 Data32;
  
  SystemMiscCfg = PcdGet32(PcdSystemMiscConfig);
  Data32        = SYS_MISC_CFG_DUAL_VGA | SYS_MISC_CFG_PRI_VGA_IGD;
  if((SystemMiscCfg & Data32) == Data32){
    return TRUE;
  }  
...
BOOLEAN IsThisPciDeviceNeedDegrade(PCI_IO_DEVICE *PciIo)
{
  ...
  if(PciIo->RomSize){
    ...
    if(ClassCode[2] == 3 && PciIo->BusNumber != 0 && IsDualVGAPrimaryIgd()){
      Rc = FALSE;
    }     
  }  
  ...
  )
{
  ...
  DEBUG((EFI_D_INFO, __FUNCTION__" (%X,%X,%X) Dec:%X\n", \
    Bridge->BusNumber, Bridge->DeviceNumber, Bridge->FunctionNumber, Bridge->Decodes));  
  ...
  while (ChildDeviceLink != NULL && ChildDeviceLink != &Bridge->ChildList) {
    ...
    if (IsThisPciDeviceNeedDegrade(Temp)) {
      ...
      if (!IsListEmpty (&PMem64Node->ChildList)) {      
        ...
        while (ChildNodeLink != &PMem64Node->ChildList) {
          ...
          if (TempNode->PciDev == Temp) {
            DEBUG((EFI_D_INFO, "PMem64 -> PMem32\n"));            
            RemoveEntryList (ChildNodeLink);
