  ...
  )
{
  ...
  //
  AllOnes &= 0xFFFFF800;
  DEBUG((EFI_D_INFO, "%a() AllOnes:%X\n", __FUNCTION__, AllOnes));	
  ...
  )
{
  ...
  do {
    ...
    if (RomHeader->Signature != PCI_EXPANSION_ROM_HEADER_SIGNATURE) {
      DEBUG((EFI_D_INFO, "BadOpRomSign:%X @+%X\n", RomHeader->Signature, RomBarOffset - RomBar));			
  ...
  if (CodeType == PCI_CODE_TYPE_PCAT_IMAGE) {
    DEBUG((EFI_D_INFO, "RomImageSize:%X %X\n",RomImageSize, ((EFI_LEGACY_EXPANSION_ROM_HEADER*)RomHeader)->Size512 * 512));			
