  ...
  while (TRUE) {
    ...
    while (PciResAlloc->GetNextRootBridge (PciResAlloc, &RootBridgeHandle) == EFI_SUCCESS) {
      ...
      if (!ReAllocate) {
        ...
        //
        DEBUG((EFI_D_INFO, "MaxOptionRomSize:0x%lX, Mem32Bridge->Length:0x%lX, Mem32Bridge->Alignment:0x%lX\n", \
                           MaxOptionRomSize, Mem32Bridge->Length, Mem32Bridge->Alignment));
...
// OptionRom may need less Rom Size, but Alighment may be more.
// NCR SCSI Card(00561000) needs 0x200000 MMIO, alignment is 2^21
// Mem32 needs 0x300000, alignment is 2^20
        if (MaxOptionRomSize && Mem32Bridge->Alignment < MaxOptionRomSize - 1) {
  ...
  //
  // Raise the EFI_IOB_PC_ENABLE status code
  //
  REPORT_STATUS_CODE (
    EFI_PROGRESS_CODE,
    (EFI_IO_BUS_PCI | EFI_IOB_PCI_RES_ASSIGN)
    );
