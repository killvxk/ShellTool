...
enum {
    ...
    //
    USB_BOT_RECV_CSW_RETRY       = 2,

    //
    // Usb Bot wait device reset complete, set by experience
    //
    USB_BOT_RESET_DEVICE_STALL   = 200 * USB_MASS_1_MILLISECOND,

    //
    // Usb Bot transport timeout, set by experience
    //
    USB_BOT_SEND_CBW_TIMEOUT     = 6 * USB_MASS_1_SECOND,
    USB_BOT_RECV_CSW_TIMEOUT     = 6 * USB_MASS_1_SECOND,
    USB_BOT_RESET_DEVICE_TIMEOUT = 6 * USB_MASS_1_SECOND,
};
...
///
/// The CSW (Command Status Wrapper) structures used by the USB BOT protocol.
///
typedef struct {
    UINT32              Signature;
    UINT32              Tag;
