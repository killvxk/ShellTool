  ...
  )
{
    ...
    CmdBuf = UsbAllocatePool (CmdLen);
    if (CmdBuf == NULL) {
        UsbFreePool (Request, sizeof(EFI_USB_DEVICE_REQUEST));
        return EFI_OUT_OF_RESOURCES;
    }

    CopyMem (CmdBuf, Cmd, CmdLen);
  ...
  )
{
    ...
    UINT32                    TransStatus;
    INTN                      Retry;
    USB_CBI_STATUS            *UsbBuf;

    UsbBuf = UsbAllocatePool (sizeof(USB_CBI_STATUS));
    if (UsbBuf == NULL)
        return EFI_OUT_OF_RESOURCES;
    ...
    CopyMem (Result, UsbBuf, sizeof(USB_CBI_STATUS));
    UsbFreePool (UsbBuf, sizeof(USB_CBI_STATUS));
    return Status;
}
  ...
  )
{
    ...
    *CmdStatus  = USB_MASS_CMD_SUCCESS;
    ...
    if (EFI_ERROR (Status)) {
        ...
        USB_DEBUG((EFI_D_ERROR, "UsbCbiExecCommand: UsbCbiSendCommand (%r)\n",Status));
        return Status;
    }
    ...
    if (UsbCbi->InterruptEndpoint == NULL) {
        USB_DEBUG((EFI_D_ERROR, "UsbCbiExecCommand: UsbCbiDataTransfer (%r)\n",Status));
        return Status;
    }
    ...
    if (EFI_ERROR (Status)) {
        USB_DEBUG((EFI_D_ERROR, "UsbCbiExecCommand: UsbCbiGetStatus (%r)\n",Status));
        return Status;
    }
