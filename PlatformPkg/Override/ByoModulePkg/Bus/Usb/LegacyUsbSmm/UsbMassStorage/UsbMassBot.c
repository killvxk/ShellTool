...
)
{
    ...
    if (EFI_ERROR (Status)) {
        USB_DEBUG((EFI_D_ERROR, "UsbBotInit: Get invalid BOT interface (%r)\n", Status));
        goto ON_ERROR;
    }
    ...
    if ((UsbBot->BulkInEndpoint == NULL) || (UsbBot->BulkOutEndpoint == NULL)) {
        USB_DEBUG((EFI_D_ERROR, "UsbBotInit: In/Out Endpoint invalid\n"));
        Status = EFI_UNSUPPORTED;
        goto ON_ERROR;
    }
...
)
{
    ...
    Cbw = UsbAllocatePool (sizeof(USB_BOT_CBW));
    if (Cbw == NULL)
        return EFI_OUT_OF_RESOURCES;
    ...
    UsbFreePool (Cbw, sizeof(USB_BOT_CBW));
    return Status;
}
  ...
  )
{
    ...
    if (EFI_ERROR (Status)) {
        USB_DEBUG((EFI_D_ERROR, "UsbBotDataTransfer: (%r)\n", Status));
        if (USB_IS_ERROR (Result, EFI_USB_ERR_STALL)) {
            USB_DEBUG((EFI_D_ERROR, "UsbBotDataTransfer: DataIn Stall\n"));
  ...
  )
{
    ...
    Csw = UsbAllocatePool (sizeof(USB_BOT_CSW));
    if (Csw == NULL)
        return EFI_OUT_OF_RESOURCES;
    ...
    *CmdStatus = USB_BOT_COMMAND_ERROR;
    ...
    for (Index = 0; Index < USB_BOT_RECV_CSW_RETRY; Index++) {
        ...
        if (EFI_ERROR(Status)) {
            USB_DEBUG((EFI_D_ERROR, "UsbBotGetStatus (%r)\n", Status));
            if (USB_IS_ERROR (Result, EFI_USB_ERR_STALL)) {
                USB_DEBUG((EFI_D_ERROR, "UsbBotGetStatus: DataIn Stall\n"));
        ...
        if (Csw->Signature != USB_BOT_CSW_SIGNATURE) {
            ...
            //
            USB_DEBUG((EFI_D_ERROR, "UsbBotGetStatus: Device return a invalid signature\n"));
            ...
            //
            USB_DEBUG((EFI_D_ERROR, "UsbBotGetStatus: Device return a phase error\n"));
            Status = UsbBotResetDevice (UsbBot, FALSE);
            break;
        } else {
    ...
    UsbFreePool (Csw, sizeof(USB_BOT_CSW));
    return Status;
}
...
)
{
    ...
    *CmdStatus  = USB_MASS_CMD_FAIL;
    ...
    if (EFI_ERROR (Status)) {
        USB_DEBUG((EFI_D_ERROR, "UsbBotExecCommand: UsbBotSendCommand (%r)\n", Status));
        return Status;
    }
    ...
    if (EFI_ERROR (Status)) {
        USB_DEBUG((EFI_D_ERROR, "UsbBotExecCommand: UsbBotGetStatus (%r)\n", Status));
        return Status;
    }
  ...
  )
{
    ...
    if (EFI_ERROR (Status)) {
        USB_DEBUG((EFI_D_ERROR, "UsbBotResetDevice: (%r)\n", Status));
...
)
{
    ...
    if (EFI_ERROR (Status)) {
        USB_DEBUG((EFI_D_ERROR, "UsbBotGetMaxLun: (%r)\n", Status));
        UsbFreePool (Data, 0x01);
        UsbFreePool (Request, sizeof(EFI_USB_DEVICE_REQUEST));
        return Status;
    }
