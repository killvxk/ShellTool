  ...
  )
{
  ...
  if (XhcIsHalt (Xhc) || XhcIsSysError (Xhc)) {
    XHCI_DEBUG ((EFI_D_ERROR, "XhcCmdTransfer: HC is halted\n"));
    goto ON_EXIT;
  }
  ...
  if (Urb == NULL) {
    XHCI_DEBUG ((EFI_D_ERROR, "XhcCmdTransfer: failed to create URB\n"));
...
--*/
{
    ...
    EFI_LIST_FOR_EACH_SAFE (Entry, Next, &Xhc->AsyncIntOnlyTransfers) {
        Urb = EFI_LIST_CONTAINER (Entry, URB, UrbList);
        //                                                                                                           //
        // For AsynIntOnly
        //
        if (Urb->DelayRecord != 0) {
            Urb->DelayCount--;
            if (Urb->DelayCount == 0) {
                Urb->DelayCount = Urb->DelayRecord;

                (Urb->Callback) (0, 0, Urb->Context, 0);
            }
        }
    }
}
...
/**
  Delete a single asynchronous interrupt only transfer for
  ...
  )
{
  ...
  EFI_LIST_FOR_EACH_SAFE (Entry, Next, &Xhc->AsyncIntOnlyTransfers) {
    ...
    if ((Urb->Ep.BusAddr == BusAddr) &&
        ...
        (Urb->Ep.Type == Type)) {
        //
        // This is a async interrupt only request, we need to continue
...
**/
URB*
XhcCreateAsyncInterruptOnlyUrb (
  IN USB_XHCI_INSTANCE                  *Xhc,
  IN UINT8                              BusAddr,
  IN UINT8                              EpAddr,
  IN UINT8                              DevSpeed,
  ...
  )
{
  ...
  switch (EPType) {
    ...
    default:
      XHCI_DEBUG ((EFI_D_INFO, "Not supported EPType 0x%x!\n",EPType));
      ASSERT (FALSE);
      break;
  }
  ...
  )
{
  ...
  Dcbaa   = UsbAllocatePage (EFI_SIZE_TO_PAGES (Entries));
  ASSERT (Dcbaa != NULL);
  ...
  if (MaxScratchpadBufs != 0) {
    ScratchBuf = UsbAllocatePage (EFI_SIZE_TO_PAGES (MaxScratchpadBufs * sizeof (UINT64)));
    ...
    for (Index = 0; Index < MaxScratchpadBufs; Index++) {
      ScratchEntryBuf = UsbAllocatePage (EFI_SIZE_TO_PAGES (Xhc->PageSize));
  ...
  XHCI_DEBUG ((EFI_D_INFO, "XhcInitSched:DCBAA=0x%x\n", (UINT64)(UINTN)Xhc->DCBAA));
  ...
  XHCI_DEBUG ((EFI_D_INFO, "XhcInitSched:XHC_CRCR=0x%x\n", Xhc->CmdRing.RingSeg0));
  ...
  XHCI_DEBUG ((EFI_D_INFO, "XhcInitSched:XHC_EVENTRING=0x%x\n", Xhc->EventRing.EventRingSeg0));
}
  ...
  )
{
  ...
  XHCI_DEBUG ((EFI_D_INFO, "Recovery Halted Slot = %x,Dci = %x\n", SlotId, Dci));
  ...
  )
{
  ...
  Buf = UsbAllocatePage (EFI_SIZE_TO_PAGES (sizeof (TRB_TEMPLATE) * EVENT_RING_TRB_NUMBER));
  ASSERT (Buf != NULL);
  ...
  Buf = UsbAllocatePage (EFI_SIZE_TO_PAGES (sizeof (EVENT_RING_SEG_TABLE_ENTRY) * ERST_NUMBER));
  ASSERT (Buf != NULL);
  ASSERT (((UINTN) Buf & 0x3F) == 0);
  ...
  )
{
  ...
  Buf = UsbAllocatePage (EFI_SIZE_TO_PAGES (sizeof (TRB_TEMPLATE) * TrbNum));
  ASSERT (Buf != NULL);
  ASSERT (((UINTN) Buf & 0x3F) == 0);
...
)
{
  ...
  for (Index = 0; Index < ERST_NUMBER; Index++) {
    ...
    if(RingBuf != NULL) {
      UsbFreePage (RingBuf, EFI_SIZE_TO_PAGES (sizeof (TRB_TEMPLATE) * EVENT_RING_TRB_NUMBER));
      ZeroMem (EventRingPtr, sizeof (EVENT_RING_SEG_TABLE_ENTRY));
    }
  }

  UsbFreePage (TablePtr, EFI_SIZE_TO_PAGES (sizeof (EVENT_RING_SEG_TABLE_ENTRY) * ERST_NUMBER));
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  if (Xhc->ScratchBuf != NULL) {
    ...
    for (Index = 0; Index < Xhc->MaxScratchpadBufs; Index++) {
      UsbFreePage ((VOID*)(UINTN)*ScratchBuf++, EFI_SIZE_TO_PAGES (Xhc->PageSize));
    }
    UsbFreePage (Xhc->ScratchBuf, EFI_SIZE_TO_PAGES (Xhc->MaxScratchpadBufs * sizeof (UINT64)));
  }

  if (Xhc->DCBAA != NULL) {
    UsbFreePage (Xhc->DCBAA, EFI_SIZE_TO_PAGES((Xhc->MaxSlotsEn + 1) * sizeof(UINT64)));
    Xhc->DCBAA = NULL;
  }

  if (Xhc->CmdRing.RingSeg0 != NULL){
    UsbFreePage (Xhc->CmdRing.RingSeg0, EFI_SIZE_TO_PAGES (sizeof (TRB_TEMPLATE) * CMD_RING_TRB_NUMBER));
  ...
  for (Index = 0; Index < Xhc->EventRing.TrbNumber; Index++) {
    ...
    switch (EvtTrb->Completecode) {
      case TRB_COMPLETION_STALL_ERROR:
        ...
        XHCI_DEBUG ((EFI_D_ERROR, "XhcCheckUrbResult: STALL_ERROR! Completecode = %x\n",EvtTrb->Completecode));
        break;

      case TRB_COMPLETION_BABBLE_ERROR:
        CheckedUrb->Result  |= EFI_USB_ERR_BABBLE;
        CheckedUrb->Finished = TRUE;
        XHCI_DEBUG ((EFI_D_ERROR, "XhcCheckUrbResult: BABBLE_ERROR! Completecode = %x\n",EvtTrb->Completecode));
        break;

      case TRB_COMPLETION_DATA_BUFFER_ERROR:
        CheckedUrb->Result  |= EFI_USB_ERR_BUFFER;
        CheckedUrb->Finished = TRUE;
        XHCI_DEBUG ((EFI_D_ERROR, "XhcCheckUrbResult: ERR_BUFFER! Completecode = %x\n",EvtTrb->Completecode));
        break;

      case TRB_COMPLETION_USB_TRANSACTION_ERROR:
        CheckedUrb->Result  |= EFI_USB_ERR_TIMEOUT;
        CheckedUrb->Finished = TRUE;
        XHCI_DEBUG ((EFI_D_ERROR, "XhcCheckUrbResult: TRANSACTION_ERROR! Completecode = %x\n",EvtTrb->Completecode));
        break;

      case TRB_COMPLETION_SHORT_PACKET:
      case TRB_COMPLETION_SUCCESS:
        if (EvtTrb->Completecode == TRB_COMPLETION_SHORT_PACKET) {
          XHCI_DEBUG ((EFI_D_ERROR, "XhcCheckUrbResult: short packet happens!\n"));
        }
      ...
      default:
        XHCI_DEBUG ((EFI_D_ERROR, "Transfer Default Error Occur! Completecode = 0x%x!\n",EvtTrb->Completecode));
        CheckedUrb->Result  |= EFI_USB_ERR_TIMEOUT;
        CheckedUrb->Finished = TRUE;
        break;
    }
  ...
  )
{
  ...
  XHCI_DEBUG ((EFI_D_INFO, "Enable Slot Successfully, The Slot ID = 0x%x\n", EvtTrb->SlotId));
  ...
  //
  InputContext = UsbAllocatePage (EFI_SIZE_TO_PAGES (sizeof (INPUT_CONTEXT)));
  ...
  //
  EndpointTransferRing = UsbAllocatePool (sizeof (TRANSFER_RING));
  ...
  //
  OutputContext = UsbAllocatePage (EFI_SIZE_TO_PAGES (sizeof (DEVICE_CONTEXT)));
  ...
  XHCI_DEBUG ((EFI_D_INFO, "    Address %d assigned successfully\n", DeviceAddress));
  ...
  )
{
  ...
  // 1) Allocate an Input Context data structure (6.2.5) and initialize all fields to '0'.
  //
  InputContext = UsbAllocatePage (EFI_SIZE_TO_PAGES (sizeof (INPUT_CONTEXT_64)));
  ASSERT (InputContext != NULL);
  ASSERT (((UINTN) InputContext & 0x3F) == 0);
  ...
  // 6) Allocate the Output Device Context data structure (6.2.1) and initialize it to '0'.
  //
  OutputContext = UsbAllocatePage (EFI_SIZE_TO_PAGES (sizeof (DEVICE_CONTEXT_64)));
  ASSERT (OutputContext != NULL);
  ASSERT (((UINTN) OutputContext & 0x3F) == 0);
  ...
  )
{
  ...
  for (Index = 0; Index < MaxUsbDevContext; Index++) {
    ...
    if (EFI_ERROR (Status)) {
      XHCI_DEBUG ((EFI_D_ERROR, "XhcDisableSlotCmd: failed to disable child, ignore error\n"));
      Xhc->UsbDevContext[Index + 1].SlotId = 0;
    }
  }
  ...
  //
  XHCI_DEBUG ((EFI_D_INFO, "Disable device slot %d!\n", SlotId));
  ...
  for (Index = 0; Index < 31; Index++) {
    if (Xhc->UsbDevContext[SlotId].EndpointTransferRing[Index] != NULL) {
      ...
      if (RingSeg != NULL) {
        UsbFreePage (RingSeg, EFI_SIZE_TO_PAGES (sizeof (TRB_TEMPLATE) * TR_RING_TRB_NUMBER));
      }
      UsbFreePool (Xhc->UsbDevContext[SlotId].EndpointTransferRing[Index], sizeof (TRANSFER_RING));
    }
  }
  ...
  if (Xhc->UsbDevContext[SlotId].InputContext != NULL) {
    UsbFreePage (Xhc->UsbDevContext[SlotId].InputContext, EFI_SIZE_TO_PAGES (sizeof (INPUT_CONTEXT)));
  }

  if (Xhc->UsbDevContext[SlotId].OutputContext != NULL) {
    UsbFreePage (Xhc->UsbDevContext[SlotId].OutputContext, EFI_SIZE_TO_PAGES (sizeof (DEVICE_CONTEXT)));
  }
  //
  ...
  )
{
  ...
  // Construct the disable slot command
  //
  XHCI_DEBUG ((EFI_D_INFO, "Disable device slot %d with cmd63!\n", SlotId));
  ...
  if (Xhc->UsbDevContext[SlotId].InputContext != NULL) {
    UsbFreePage (Xhc->UsbDevContext[SlotId].InputContext, EFI_SIZE_TO_PAGES (sizeof (INPUT_CONTEXT_64)));
  }

  if (Xhc->UsbDevContext[SlotId].OutputContext != NULL) {
    UsbFreePage (Xhc->UsbDevContext[SlotId].OutputContext, EFI_SIZE_TO_PAGES (sizeof (DEVICE_CONTEXT_64)));
  }
  //
  // Doesn't zero the entry because XhcAsyncInterruptTransfer() may be invoked to remove the established
  // asynchronous interrupt pipe after the device is disabled. It needs the device address mapping info to
  ...
  )
{
  ...
  XHCI_DEBUG ((EFI_D_INFO, "Configure Endpoint\n"));
  Status = XhcCmdTransfer (
             Xhc,
  ...
  )
{
  ...
  XHCI_DEBUG ((EFI_D_INFO, "Evaluate context\n"));
  Status = XhcCmdTransfer (
             Xhc,
  ...
  )
{
  ...
  CmdTrbCfgEP.Type     = TRB_TYPE_CON_ENDPOINT;
  CmdTrbCfgEP.SlotId   = Xhc->UsbDevContext[SlotId].SlotId;
  XHCI_DEBUG ((EFI_D_INFO, "Configure Hub Slot Context\n"));
  Status = XhcCmdTransfer (
              Xhc,
              (TRB_TEMPLATE *) (UINTN) &CmdTrbCfgEP,
              XHC_GENERIC_TIMEOUT,
...
)
{
  ...
  while (CCS == pEventRing->EventRingCCS) {
    ...
    if (EvtTrb->Completcode == TRB_COMPLETION_STALL_ERROR) {
       XHCI_DEBUG((EFI_D_INFO, "Recovery from halt ! \n"));
    }
