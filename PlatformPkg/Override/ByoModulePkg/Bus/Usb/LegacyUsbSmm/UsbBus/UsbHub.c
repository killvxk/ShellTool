  ...
  )
{
  ...
  if (Result != EFI_USB_NOERROR) {
    ...
    if (EFI_ERROR (Status)) {
      USB_DEBUG (( EFI_D_ERROR, "UsbOnHubInterrupt: failed to remove async transfer - %r\n", Status));
      return Status;
    }
    ...
    if (EFI_ERROR (Status)) {
      USB_DEBUG (( EFI_D_ERROR, "UsbOnHubInterrupt: failed to submit new async transfer - %r\n", Status));
    }
  ...
  )
{
  ...
  if (Index == NumEndpoints) {
    USB_DEBUG (( EFI_D_ERROR, "UsbHubInit: no interrupt endpoint found for hub %d\n", HubDev->Address));
  ...
  if (EFI_ERROR (Status)) {
    USB_DEBUG (( EFI_D_ERROR, "UsbHubInit: failed to read HUB descriptor %r\n", Status));
    return Status;
  }

  HubIf->NumOfPort = HubDesc.NumPorts;

  USB_DEBUG (( EFI_D_INFO, "UsbHubInit: hub %d has %d ports\n", HubDev->Address,HubIf->NumOfPort));
  ...
  if (HubIf->Device->Speed == EFI_USB_SPEED_SUPER) {
    ...
    USB_DEBUG ((EFI_D_INFO, "UsbHubInit: Set Hub Depth as 0x%x\n", Depth));
    ...
    Stall (HubDesc.PwrOn2PwrGood * USB_SET_PORT_POWER_STALL);
  ...
  if (EFI_ERROR (Status)) {
    USB_DEBUG (( EFI_D_ERROR, "UsbHubInit: failed to queue interrupt transfer for hub %d - %r\n",
                HubDev->Address, Status));

    return Status;
  }

  USB_DEBUG (( EFI_D_INFO, "UsbHubInit: hub %d initialized\n", HubDev->Address));
  return Status;
}
  ...
  )
{
  ...
  USB_DEBUG (( EFI_D_INFO, "UsbHubRelease: hub device %d released\n", HubIf->Device->Address));
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  USB_DEBUG (( EFI_D_INFO, "UsbRootHubInit: root hub %p - max speed %d, %d ports\n",
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    USB_DEBUG (( EFI_D_ERROR, "UsbRootHubResetPort: failed to start reset on port %d\n", Port));
    return Status;
  }
  ...
  if (EFI_ERROR (Status)) {
    USB_DEBUG (( EFI_D_ERROR, "UsbRootHubResetPort: failed to clear reset on port %d\n", Port));
    return Status;
  }
  ...
  if (Index == USB_WAIT_PORT_STS_CHANGE_LOOP) {
    USB_DEBUG ((EFI_D_ERROR, "UsbRootHubResetPort: reset not finished in time on port %d\n", Port));
    return EFI_TIMEOUT;
  }
  ...
  if (!USB_BIT_IS_SET (PortState.PortStatus, USB_PORT_STAT_ENABLE)) {
    ...
    if (RootIf->MaxSpeed == EFI_USB_SPEED_HIGH) {
      USB_DEBUG (( EFI_D_ERROR, "UsbRootHubResetPort: release low/full speed device (%d) to UHCI\n", Port));
      ...
      if (EFI_ERROR (Status)) {
        USB_DEBUG (( EFI_D_ERROR, "UsbRootHubResetPort: failed to enable port %d for UHCI\n", Port));
        return Status;
      }
...
{
  USB_DEBUG (( EFI_D_INFO, "UsbRootHubRelease: root hub released for hub %p\n", HubIf));
  return EFI_SUCCESS;
}
