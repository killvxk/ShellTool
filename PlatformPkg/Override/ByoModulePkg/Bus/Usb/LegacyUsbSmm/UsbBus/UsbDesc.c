  ...
  )
{
  ...
  *Consumed = 0;
  ...
  if ((Len <= Offset)      || (Len < Offset + DescLen) ||
    ...
    USB_DEBUG (( EFI_D_ERROR, "UsbCreateDesc: met mal-format descriptor\n"));
    return NULL;
  }
  ...
  )
{
  ...
  *Consumed = 0;
  ...
  if (Setting == NULL) {
    USB_DEBUG (( EFI_D_ERROR, "UsbParseInterfaceDesc: failed to create interface descriptor\n"));
    return NULL;
  }
  ...
  USB_DEBUG (( EFI_D_INFO, "UsbParseInterfaceDesc: interface %d(setting %d) has %d endpoints\n",
  ...
  for (Index = 0; Index < NumEp; Index++) {
    ...
    if (Ep == NULL) {
      USB_DEBUG (( EFI_D_ERROR, "UsbParseInterfaceDesc: failed to create endpoint(index %d)\n", (UINT32)Index));
      goto ON_ERROR;
    }
  ...
  )
{
  ...
  USB_DEBUG (( EFI_D_INFO, "UsbParseConfigDesc: config %d has %d interfaces\n",
  ...
  while (Len >= sizeof (EFI_USB_INTERFACE_DESCRIPTOR)) {
    ...
    if (Setting == NULL) {
      USB_DEBUG (( EFI_D_ERROR, "UsbParseConfigDesc: failed to parse interface setting\n"));
      goto ON_ERROR;

    } else if (Setting->Desc.InterfaceNumber >= NumIf) {
      USB_DEBUG (( EFI_D_ERROR, "UsbParseConfigDesc: mal-formated interface descriptor\n"));
  ...
  )
{
  ...
  if (Length != 0 && Buf != NULL) {
    UsbBuf = UsbAllocatePool (Length);
    if (UsbBuf == NULL) {
      UsbFreePool (DevReq, sizeof(EFI_USB_DEVICE_REQUEST));
      return EFI_OUT_OF_RESOURCES;
    }
    CopyMem (UsbBuf, Buf, Length);
  } else {
    UsbBuf = Buf;
  }
  ...
  if (Length != 0 && Buf != NULL) {
    if (!EFI_ERROR(Status)) {
      CopyMem (Buf, UsbBuf, Length);
    }
    UsbFreePool (UsbBuf, Length);
  }
  UsbFreePool (DevReq, sizeof (EFI_USB_DEVICE_REQUEST));
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    USB_DEBUG (( EFI_D_ERROR, "UsbGetOneConfig: failed to get descript length(%d) %r\n",
                Desc.TotalLength, Status));

    return NULL;
  }

  USB_DEBUG (( EFI_D_INFO, "UsbGetOneConfig: total length is %d\n", Desc.TotalLength));
  ...
  if (EFI_ERROR (Status)) {
    USB_DEBUG (( EFI_D_ERROR, "UsbGetOneConfig: failed to get full descript %r\n", Status));
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    USB_DEBUG (( EFI_D_ERROR, "UsbBuildDescTable: failed to get device descriptor - %r\n", Status));
    return Status;
  }
  ...
  USB_DEBUG (( EFI_D_INFO, "UsbBuildDescTable: device has %d configures\n", NumConfig));
  ...
  for (Index = 0; Index < NumConfig; Index++) {
    ...
    if (Config == NULL) {
      USB_DEBUG (( EFI_D_ERROR, "UsbBuildDescTable: failed to get configure (index %d)\n", Index));
    ...
    if (ConfigDesc == NULL) {
      USB_DEBUG (( EFI_D_ERROR, "UsbBuildDescTable: failed to parse configure (index %d)\n", Index));
  ...
  if (EFI_ERROR (Status)) {
    USB_DEBUG (( EFI_D_ERROR, "UsbBuildDescTable: get language ID table %r\n", Status));
  }
  ...
  )
{
  ...
  Status = UsbIo->UsbControlTransfer (
                    ...
                    DevReq,
                    EfiUsbNoData,
                    USB_CLEAR_FEATURE_REQUEST_TIMEOUT / USB_BUS_1_MILLISECOND,
                    NULL,
                    0,
                    &UsbResult
                    );
