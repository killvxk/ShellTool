  ...
  )
{
  ...
  IoWrite16(PMIO_REG(PMIO_GBLSTS_REG), PMIO_GBLSTS_SWSMI);  // Clear the APM SMI Status Bit
  IoOr16(PMIO_REG(PMIO_GBLCTRL_REG), PMIO_GC_SMIACTIVE);      // Set the EOS Bit
}

BOOLEAN IsSmiSourceActive()
{
  BOOLEAN  rc;
  rc = (IoRead16(PMIO_REG(PMIO_GBLCTRL_REG)) & PMIO_GC_SMIACTIVE)?TRUE:FALSE;
	return rc;
}
