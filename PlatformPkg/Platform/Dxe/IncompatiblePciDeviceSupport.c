...
STATIC UINT64 gIgdIncompatibleData[] = {
  DEVICE_INF_TAG,
  PCI_DEVICE_ID(0x1106, 0x3A02, DEVICE_ID_NOCARE, DEVICE_ID_NOCARE, DEVICE_ID_NOCARE),
  DEVICE_RES_TAG,
  PCI_BAR_TYPE_MEM,         // ResType
  PCI_ACPI_UNUSED,          // GenFlag
  PCI_ACPI_UNUSED,          // SpecificFlag
  PCI_ACPI_UNUSED,          // AddrSpaceGranularity
  PCI_ACPI_UNUSED,          // RangeMin
  0x0000FFF,                // RangeMax --> Alignment
  1,                        // Bar index
  0x0001000,                // AddrLen
};

STATIC UINT64 gSataIncompatibleData[] = {
  DEVICE_INF_TAG,
  PCI_DEVICE_ID(0x1106, 0xFFFF, DEVICE_ID_NOCARE, DEVICE_ID_NOCARE, DEVICE_ID_NOCARE),
  DEVICE_RES_TAG,
  PCI_BAR_TYPE_IO,          // ResType
  PCI_ACPI_UNUSED,          // GenFlag
  PCI_ACPI_UNUSED,          // SpecificFlag
  PCI_ACPI_UNUSED,          // AddrSpaceGranularity
  PCI_ACPI_UNUSED,          // RangeMin
  0x0000001F,               // RangeMax --> Alignment
  4,                        // Bar index
  0x00000020,               // AddrLen  
};

STATIC UINT64 gSata3IncompatibleData[] = {
  DEVICE_INF_TAG,
  PCI_DEVICE_ID(0x1106, 0xFFFF, DEVICE_ID_NOCARE, DEVICE_ID_NOCARE, DEVICE_ID_NOCARE),
  DEVICE_RES_TAG,
  PCI_BAR_TYPE_IO,          // ResType
...
UINT64 *mIncompatiblePciDeviceList = NULL;



STATIC UINT16 GetPciIoBarSize(UINTN IoBarPciAddr)
{
  UINT32  BarVal;
  UINT16  BarSize;  

  BarVal = MmioRead32(IoBarPciAddr);
  MmioWrite32(IoBarPciAddr, 0xFFFFFFFF);
  BarSize = MmioRead16(IoBarPciAddr) & (~BIT0);
  BarSize = (~BarSize) + 1;
  MmioWrite32(IoBarPciAddr, BarVal);

  DEBUG((EFI_D_INFO, "IoBarSize:0x%X\n", BarSize));
  return BarSize;
}


EFI_STATUS InitIncompatibleTable(VOID)
{
  UINTN   TotalSize;
  UINT8   *Buffer;
   //ERC20161103 No ignore IGD -S
   //UINT16  VendorId;
  // UINTN	 PciBase;
   //ERC20161103 No ignore IGD -E
  

  TotalSize = sizeof(gIgdIncompatibleData) + 
              sizeof(gSataIncompatibleData) +
              sizeof(gSata3IncompatibleData) +
              sizeof(UINT64);                      // end
  Buffer = (UINT8*)AllocatePool(TotalSize + sizeof(UINT64));
  ASSERT(Buffer != NULL);
  if(Buffer == NULL){
    return EFI_OUT_OF_RESOURCES;
  }  
  
  Buffer = (UINT8*)ALIGN_POINTER(Buffer, sizeof(UINT64));
  mIncompatiblePciDeviceList = (UINT64*)Buffer;
//ERC20161103 No ignore IGD -S
/*
  PciBase  = IGD_PCI_REG(PCI_VID_REG);
  VendorId = MmioRead16(PciBase);
  if(VendorId != 0xFFFF){
    gIgdIncompatibleData[1] = VendorId;
    gIgdIncompatibleData[2] = MmioRead16(PciBase+PCI_DID_REG);
    CopyMem(Buffer, gIgdIncompatibleData, sizeof(gIgdIncompatibleData));
    Buffer += sizeof(gIgdIncompatibleData);
  }
  */
  //ERC20161103 No ignore IGD -E
  
#if 0  
  PciBase  = SATA_PCI_REG(PCI_VID_REG);
  VendorId = MmioRead16(PciBase);
  if(VendorId != 0xFFFF &&
    MmioRead8(PciBase+PCI_SCC_REG) == PCI_SCC_IDE &&
    GetPciIoBarSize(PciBase+0x20) < 0x20){
    gSataIncompatibleData[1] = VendorId;
    gSataIncompatibleData[2] = MmioRead16(PciBase + PCI_DID_REG);
    CopyMem(Buffer, gSataIncompatibleData, sizeof(gSataIncompatibleData));
    Buffer += sizeof(gSataIncompatibleData);    
  }

  PciBase  = SATA3_PCI_REG(PCI_VID_REG);
  VendorId = MmioRead16(PciBase);
  if(VendorId != 0xFFFF && 
    MmioRead8(PciBase+PCI_SCC_REG) == PCI_SCC_IDE &&
    GetPciIoBarSize(PciBase+0x20) < 0x20){
    gSata3IncompatibleData[1] = VendorId;
    gSata3IncompatibleData[2] = MmioRead16(PciBase+PCI_DID_REG);
    CopyMem(Buffer, gSata3IncompatibleData, sizeof(gSata3IncompatibleData));
    Buffer += sizeof(gSata3IncompatibleData);     
  }  
#endif

  *(UINT64*)Buffer = LIST_END_TAG;
  ...
  * (VOID **) Configuration = NULL;
  ...
  while (*ListPtr != LIST_END_TAG) {
    ...
    case DEVICE_INF_TAG:
      ...
      DEBUG((EFI_D_INFO, "[PatchPci] Found %04X%04X\n", DeviceId, VendorId));
